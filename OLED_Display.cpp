...
extern const uint8_t send_packet_interval;
int nextTxTime;
...
// converts countDown time to a String: "hh:mm:ss"
String OLED_Display::countDown_time( void ) {
  String txt;

int s,m,h;
int GPSserialTime = GPS.hour*3600 + GPS.minute*60 + GPS.seconds; 

  if (GPSserialTime > nextTxTime) {
    s = nextTxTime + ((2400*3600) - GPSserialTime);
  }
  else {
    s = nextTxTime - GPSserialTime ;
  }
