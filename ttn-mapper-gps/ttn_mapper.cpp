...
//  display.print(display.gps_time());
  display.print(display.countDown_time());
...
//      display.print(" ");
//      display.println(GPS.longitude_fixed);
      display.addSent();
    } else {
  ...
  nextTxTime = (GPS.hour*3600 + GPS.minute*60 + GPS.seconds) + interval;
  if (nextTxTime >= 24*3600) nextTxTime -= 24*3600;
